...
public class LoadJobUtil {
    ...
    private ScheduleJob getJob(String jobGroup,Element job) throws Exception {
        ...
        String factoryType = job.attributeValue("factoryType");
        String jobClass = job.attributeValue("class");
        String method = job.attributeValue("method");
        String ifSync = job.attributeValue("ifSync");
        ...
        if(StringUtils.isNotBlank(jobGroup) && StringUtils.isNotBlank(jobName) && StringUtils.isNotBlank(factoryType) && StringUtils.isNotBlank(jobClass)) {
            sj = new ScheduleJob(jobGroup, jobName).setFactory(factoryType)
                    .setTarget(jobClass).setMethod(method).setIfSync(ifSync)
                    .setJobDesc(jobDesc);
            ...
            for (Iterator<Element> triggerIterator = triggers.iterator(); triggerIterator.hasNext();) {
                ...
                sj.addTrigger(new ScheduleTrigger(trigger
                        .attributeValue("name"))
                        .setPriority(trigger.attributeValue("priority"))
                        .setType(trigger.attributeValue("type"))
                        .setExpression(trigger.attributeValue("expression"))
                        .setStartAt(trigger.attributeValue("startAt"))
                        .setEndAt(trigger.attributeValue("endAt"))
                        .setTriggerDesc(trigger.attributeValue("desc")));
            }
